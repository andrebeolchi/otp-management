...
export class OneSignalSMSProvider implements NotificationProvider {
  async send(recipient: Recipient, _content: Content): Promise<void> {
    try {
      console.log(`SMS sent to ${recipient.value} via OneSignal: ${_content.body}`)
    } catch (error) {
      console.error('Failed to send SMS via OneSignal:', error)
